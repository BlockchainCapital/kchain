...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    ExtractQtumTX kchainTx;
    BOOST_CHECK(converter.extractionQtumTransactions(kchainTx));
    std::vector<QtumTransaction> result = kchainTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(kchaintxconverter_tests, TestingSetup)
