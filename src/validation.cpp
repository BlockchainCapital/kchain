...
bool CheckTransaction(const CTransaction& tx, CValidationState &state, bool fCheckDuplicateInputs)
{
    ...
    if (::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > MAX_TRANSACTION_BASE_SIZE ||
        ::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // kchain
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool rawTx)
{
...

    {
        ...
        if(tx.HasCreateOrCall()){
            ...
            uint64_t minGasPrice = kchainDGP.getMinGasPrice(chainActive.Tip()->nHeight + 1);
            uint64_t blockGasLimit = kchainDGP.getBlockGasLimit(chainActive.Tip()->nHeight + 1);
            size_t count = 0;
            ...
            std::vector<QtumTransaction> kchainTransactions = resultConverter.first;
            std::vector<EthTransactionParams> kchainETP = resultConverter.second;

            dev::u256 sumGas = dev::u256(0);
            dev::u256 gasAllTxs = dev::u256(0);
            for(QtumTransaction kchainTransaction : kchainTransactions){
                sumGas += kchainTransaction.gas() * kchainTransaction.gasPrice();
                ...
                if(txMinGasPrice != 0) {
                    txMinGasPrice = std::min(txMinGasPrice, kchainTransaction.gasPrice());
                } else {
                    txMinGasPrice = kchainTransaction.gasPrice();
                }
                VersionVM v = kchainTransaction.getVersion();
                if(v.format!=0)
                ...
                if(kchainTransaction.gas() < GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
                ...
                if(kchainTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
                ...
                if(kchainTransaction.gas() > UINT32_MAX)
                ...
                gasAllTxs += kchainTransaction.gas();
                ...
                if(v.rootVM!=0 && (uint64_t)kchainTransaction.gasPrice() < minGasPrice)
            ...
            if(!CheckMinGasPrice(kchainETP, minGasPrice))
                return state.DoS(100, false, REJECT_INVALID, "bad-txns-small-gasprice");

            if(count > kchainTransactions.size())
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s KCHAIN additional fees, %d delta bytes\n",
...
static DisconnectResult DisconnectBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // kchain
    globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // kchain
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path kchainDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(kchainDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(kchainDir.string(), std::ios::in | std::ios::out);
...
bool QtumTxConverter::extractionQtumTransactions(ExtractQtumTX& kchaintx){
    ...
    kchaintx = std::make_pair(resultTX, resultETP);
    return true;
}
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // kchain
    QtumDGP kchainDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setQtumSchedule(kchainDGP.getGasSchedule(pindex->nHeight + 1));
    uint32_t sizeBlockDGP = kchainDGP.getBlockSize(pindex->nHeight + 1);
    uint64_t minGasPrice = kchainDGP.getMinGasPrice(pindex->nHeight + 1);
    uint64_t blockGasLimit = kchainDGP.getBlockGasLimit(pindex->nHeight + 1);
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // kchain
...
bool static ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace)
{
    ...
    LogPrint("bench", "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * 0.001, nTimeReadFromDisk * 0.000001);
    {
        ...
        dev::h256 oldHashStateRoot(globalState->rootHash()); // kchain
        dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // kchain
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot); // kchain
            globalState->setRootUTXO(oldHashUTXORoot); // kchain
...
//////////////////////////////////////////////////////////////////////////
    ...
    for (CBlockIndex* pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev)
    {
        ...
        ///////////////////////////////////////////////////////////////////// // kchain
        uint32_t sizeBlockDGP = kchainDGP.getBlockSize(pindex->nHeight);
        dgpMaxBlockSize = sizeBlockDGP ? sizeBlockDGP : dgpMaxBlockSize;
        updateBlockSizeParams(dgpMaxBlockSize);
        /////////////////////////////////////////////////////////////////////
