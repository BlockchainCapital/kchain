...
{
    CBitcoinAddress kchainAddress(strQtumAddress);
    if(kchainAddress.IsValid()){
        CKeyID keyid;
        kchainAddress.GetKeyID(keyid);
...
bool ToQtumAddress(const std::string& strHash160, std::string& strQtumAddress)
{
    ...
    CKeyID keyid(key);
    CBitcoinAddress kchainAddress;
    kchainAddress.Set(keyid);
    if(kchainAddress.IsValid()){
        strQtumAddress = kchainAddress.ToString();
        return true;
    }
    return false;
}
