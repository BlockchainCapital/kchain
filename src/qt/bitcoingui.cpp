...
#include "titlebar.h"
#include "kchainversionchecker.h"
    ...
    platformStyle(_platformStyle)
{
    ...
    kchainVersionChecker = new QtumVersionChecker(this);

    if(fCheckForUpdates && kchainVersionChecker->newVersionAvailable())
    {
        QString link = QString("<a href=%1>%2</a>").arg(KCHAIN_RELEASES, KCHAIN_RELEASES);
...
void BitcoinGUI::createActions()
{
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and kchain: URIs)"));
    ...
    openAction->setStatusTip(tr("Open a kchain: URI or payment request"));
