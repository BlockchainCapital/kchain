...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(!uri.isValid() || uri.scheme() != QString("kchain"))
        return false;
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    if(uri.startsWith("kchain://", Qt::CaseInsensitive))
    {
        uri.replace(0, 7, "kchain:"); // KCHAIN: change string length to 7 to correctly parse kchain://
    }
...
{
    QString ret = QString("kchain:%1").arg(info.address);
    int paramCount = 0;
...
boost::filesystem::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "kchain.desktop";
    return GetAutostartDir() / strprintf("kchain-%s.lnk", chain);
}
