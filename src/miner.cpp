...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setQtumSchedule(kchainDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = kchainDGP.getBlockSize(nHeight);
    minGasPrice = kchainDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = kchainDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    for(QtumTransaction kchainTransaction : kchainTransactions){
        txGas += kchainTransaction.gas();
        if(txGas > txGasLimit) {
            // Limit the tx gas limit by the soft limit if such a limit has been specified.
            return false;
        }

        if(bceResult.usedGas + kchainTransaction.gas() > softBlockGasLimit){
            //if this transaction's gasLimit could cause block gas limit to be exceeded, then don't add it
            return false;
        }
        if(kchainTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, kchainTransactions, hardBlockGasLimit);
