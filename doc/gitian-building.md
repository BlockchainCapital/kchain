...
- [Building Qtum Core](#building-kchain-core)
...
Clone the git repositories for kchain and Gitian.
...
```bash
sudo cp /home/debian/kchain/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the kchain repository under 'Fetch and create inputs' to install sources which require
...
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the kchain repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/kchain/.git/
...
For example:
```bash
URL=https://github.com/laanwj/kchain.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit kchain=${COMMIT} --url kchain=${URL} ../kchain/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit kchain=${COMMIT} --url kchain=${URL} ../kchain/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit kchain=${COMMIT} --url kchain=${URL} ../kchain/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-kchain:
./bin/gbuild --commit kchain=${COMMIT},cpp-eth-kchain=${ETHCOMMIT} --url kchain=${URL},cpp-eth-kchain=${ETHURL} ../kchain/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the kchain git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../kchain/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/kchain
SIGPATH=/some/root/path/kchain-detached-sigs

./bin/gbuild --url kchain=${BTCPATH},signature=${SIGPATH} ../kchain/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/kchain-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/kchain-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/kchain-osx-build.assert
```
...
[kchainproject/gitian.sigs](https://github.com/kchainproject/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@kchain.org) and he will commit them.
